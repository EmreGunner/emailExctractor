...
def process_href(href, url):
    ...
    if href in checked_websites or check_url_exist_db(href) or href == url:
    ...
    try:
        ...
        if page_content:
            ...
            save_emails_to_db(href, mails, get_domain(href))  
        else:
            logging.info(f"Error with subpage {href}")
            save_errored_website(href,mails="could not reach site", error="could not reach site")
